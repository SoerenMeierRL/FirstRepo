...
1.  If some sites contain multiple biomes, why is `any()` a reasonable function for aggregating to presence–absence?
    ...
    -   Because it marks a site as having the biome present, even if other biomes exist within that site.
...
2.  What challenges arise if the positive class ("Temperate Broadleaf & Mixed Forests") is very rare or very common in the dataset?
    ...
    -   This will create a very biased model that has trouble accurately predicting new sites, as the model will most likely predict "absent" all the time if the focal biome is very rare, and vice versa for a very common biome.

3.  Why is a site-level binary response preferable to a multi-class target for this exercise?

    -   Because it lets us focus on simply predicting the presence/absence of a single focal biome instead of combinations of biomes.
...
dat <- dat[
  complete.cases(dat[, 3:22, drop = FALSE]),  # Check predictor columns
]
...
1.  When removing locations with incomplete climate data, what types of locations might be systematically excluded, and how could this bias the understanding of TBMF distribution?
    -   Locations with incomplete data sampling are usually underdeveloped/and or remote regions. The TBMF distribution will therefore only be accurate for locations with efficient and complete data sampling, and have low accuracy for other sites.
2.  Why is class balance important for evaluating model accuracy?
    -   Class imbalance might bias the model heavily towards predicting a single outcome, regardless of the predictor values. Class balance ensures that the model has enough data to accurately predict all scenarios.
3.  Which climate variables might be collinear, and why do tree-based models typically handle collinearity well?
    -   All temperature related variables might be collinear, and all precipitation variables might also be collinear (as they are different ways of measuring the same basic variable). Tree-based models handle collinearity by evaluating the importance of one predictor at a time and decide splitting based on this. As this is done for one predictor at a time, collinearity between predictor will not influence the individual assessment of predictors.
...
1.  Why does pruning usually improve out-of-sample performance even if training accuracy drops?
    -   Because by pruning, we avoid overfitting to our training sample perfectly, and we can infer more general patterns to use for predicting with new data.
2.  What does the `xerror` in the CP table represent, and how does it differ from external 10-fold CV?
    -   xerror is the average prediction error (misspecification rate) of a tree on unseen data (of all the training data) while it is being trained on the remaining training data - this process is done with many subsets/folds of the training data, and the average value across these subsets is used. In external CV, the prediction error is based on predictions on completely new data outside of the training data itself.
3.  How can the 1-SE choice be justified when describing results?
    -   The 1-SE choice avoids picking a too simple or too complex tree.
...
1.  Why is nested cross-validation necessary (both internal `xval=10` within `rpart` AND external CV)? What would go wrong with only one level?
    -   Nested cross-validation allows for selecting the most optimal model that describes and predicts all data the most accurately, even new data. With only one level, the tree would only have been evaluated on the basis of a small subset of the data and be very biased and inaccurate to the rest of the data.
2.  How should the 0.5 probability threshold used in the `acc_metrics` function be chosen depending on whether the goal is conservation planning, ecological research, or climate change prediction?
    -   The threshold should probably be lowered in conservation to capture all potential suitable sites, even with the risk of falsely predicting suitable sites - rather than missing them entirely.

    -   Ecological research should usually use the 0.5 threshold to have the most balanced and accurate general predictions.

    -   In climate change prediction, the threshold can be adjusted lower to capture new potential sites, or adjusted higher to better capture loss of sites.
3.  Does random folding violate independence assumptions when nearby locations have similar climates and biomes (spatial autocorrelation)?
    -   The random folds can potentially evaluate the model on very similar locations, and the evaluation will therefore be biased because of these dependent observations - the process is random, and therefore there is always a risk of violating the independence assumption.
...
rf_cv <- do.call(rbind, 
  lapply(folds, function(idx_test){  # Use folds from before v(idx_test <- folds[[1]])
    tr <- dat[-idx_test, ]
    te <- dat[idx_test, ]
...
1.  Why can OOB error serve as a trustworthy proxy for cross-validation in Random Forests?
    -   Because it functions similar to cross-validation by evaluating predictions on subsets of the data.
2.  How does `mtry` influence the bias–variance trade-off?
    -   Increasing mtry decreases bias (each tree more accurately describes the relationship via the strongest predictors), but increases variance, as each tree is more similar/correlated.
3.  What does the optimal `mtry` value reveal about climate-biome relationship complexity?
    -   The optimal mtry value of 4 indicates that the climate-biome relationship is best described by using 4 variables - a not too complex relationship.
4.  For conservation planning, would students recommend the interpretable CART model or the more accurate but "black box" Random Forest?
    -   For optimal planning, the most accurate results would be preferable, but to understand the underlying factors better, one could use the CART model and interpret it.
